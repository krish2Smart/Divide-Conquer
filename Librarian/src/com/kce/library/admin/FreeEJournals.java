...
/**
 * Servlet implementation class FreeEJournals
 */
@WebServlet("/FreeEJournals")
public class FreeEJournals extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FreeEJournals() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try(PrintWriter out = response.getWriter()) {
			...
			try {
				...
				ResultSet rs = null;
				rs = st.executeQuery("select * from free_e_journals where link = '"+link+"'");
				if(rs.next()) {
					li.put("code", "1");
					li.put("message", "Link already exist");
				} else {
					st.executeUpdate("insert into free_e_journals values('"+link+"')");
				}
				li.put("code", "1");
